...
// «Esta orden cambió después de volverse remisión».
//
...
export function CambioPanel({ oc, canWrite, onResuelto }: {
  oc: OCRecibidaDetalle;
  canWrite: boolean;
  onResuelto: () => void;
}) {
  ...
  if (!oc.cambio_detectado_at || !d) return null;
  ...
  async function resolver() {
    ...
    setGuardando(true);
    try {
      await apiFetch(`/api/v1/oc-recibidas/${oc.id}/cambio/resolver`, {
        method: "POST",
    ...
    <Alert tone={abierto ? "danger" : "info"}>
      <div className="space-y-3">
        ...
        <div className="text-sm">
          ...
          {oc.remision_folio ? <> · remisión <strong>{oc.remision_folio}</strong></> : null}
        </div>
